...
export class AdminsStudentListComponent implements OnInit {
  ...
  pagedUsers: PagedResult < IStudentWithInfoRow > ;
  ...
  loadPagedUsers() {
    ...
    this._adminService.getPagedStudentsInfo(paginatedRequest)
      .subscribe((pagedUsers: PagedResult < IStudentWithInfoRow > ) => {
  ...
  deleteDialog(id: number) {
    this._adminService.deleteStudent(id).subscribe(
      () => {
        this.loadPagedUsers();
      });
  }
    ...
    if (this.userForm.dirty && this.userForm.valid) {
      if (this.pageTitle == 'Add') {
        const createUser: UserForRegistrationDto = {
          ...this.userForm.value,
          type: 0,
          classId: this.userForm.value.className
        }
